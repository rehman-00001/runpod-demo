...
def file_exists(path):
    """
    Check if a file exists, including support for:
    - Regular files
    - Symbolic links (Unix/Linux/Mac)
    - Junction points (Windows)
    - Windows shortcuts (.lnk files)
    - Alternative extensions (.sft <-> .safetensors)
    """
    # Check if path exists (this follows symlinks and junctions automatically)
    if os.path.exists(path):
        return True
    
    # Check if it's a broken symlink or junction
    if os.path.islink(path):
        # It's a symlink/junction but target doesn't exist
        return False
    
    # Check for alternative file extensions (.sft <-> .safetensors)
    base_path, ext = os.path.splitext(path)
    if ext == '.sft':
        alt_path = base_path + '.safetensors'
        if os.path.exists(alt_path) or os.path.islink(alt_path):
            return True
    elif ext == '.safetensors':
        alt_path = base_path + '.sft'
        if os.path.exists(alt_path) or os.path.islink(alt_path):
            return True
    
    # On Windows, also check for .lnk shortcut files
    if sys.platform == 'win32':
        lnk_path = path + '.lnk'
        if os.path.exists(lnk_path):
            try:
                # Try to resolve Windows shortcut
                import winshell
                shortcut = winshell.shortcut(lnk_path)
                return os.path.exists(shortcut.path)
            except:
                # If winshell is not available, just check if .lnk exists
                return True
...
def download(base_model):
    ...
    if not file_exists(unet_path):
        ...
        link_type = "(symlink)" if os.path.islink(unet_path) else ""
        print(f"Base model {base_model} already exists at {unet_path} {link_type}, skipping download")
        gr.Info(f"Base model {base_model} already exists, reusing existing model")

    # download vae
    vae_folder = "models/vae"
    vae_path = os.path.join(vae_folder, "ae.sft")
    if not file_exists(vae_path):
        ...
        link_type = "(symlink)" if os.path.islink(vae_path) else ""
        print(f"VAE model already exists at {vae_path} {link_type}, skipping download")
        gr.Info(f"VAE model already exists, reusing existing model")

    # download clip
    clip_folder = "models/clip"
    clip_l_path = os.path.join(clip_folder, "clip_l.safetensors")
    if not file_exists(clip_l_path):
        ...
        link_type = "(symlink)" if os.path.islink(clip_l_path) else ""
        print(f"CLIP model already exists at {clip_l_path} {link_type}, skipping download")
        gr.Info(f"CLIP model already exists, reusing existing model")

    # download t5xxl
    t5xxl_path = os.path.join(clip_folder, "t5xxl_fp16.safetensors")
    if not file_exists(t5xxl_path):
        ...
        link_type = "(symlink)" if os.path.islink(t5xxl_path) else ""
        print(f"T5XXL model already exists at {t5xxl_path} {link_type}, skipping download")
        gr.Info(f"T5XXL model already exists, reusing existing model")
...
#        --max_grad_norm 0.0 {line_break}"""
    if vram == "30G":
        # 30G+ VRAM - Optimized for high-performance training
  ...
  --num_cpu_threads_per_process 6 {line_break}
...
with gr.Blocks(elem_id="app", theme=theme, css=css, fill_width=True) as demo:
    with gr.Tabs() as tabs:
        ...
        """, elem_classes="group_padding")
                    ...
                    vram = gr.Radio(["30G", "20G", "16G", "12G" ], value="30G", label="VRAM", interactive=True)
        ...
        """, elem_classes="group_padding")
            ...
            with gr.Accordion("Advanced options", elem_id='advanced_options', open=False):
                with gr.Row():
                    ...
                    with gr.Column(min_width=300):
                        workers = gr.Number(label="--max_data_loader_n_workers", info="Number of Workers", value=6, interactive=True)
                    with gr.Column(min_width=300):
